...
const getHighestBidsByProductId = `-- name: GetHighestBidsByProductId :one
SELECT id, product_id, bidder_id, bid_amount, created_at FROM bids
WHERE product_id = $1
ORDER BY bid_amount DESC
LIMIT 1
...
func (q *Queries) GetHighestBidsByProductId(ctx context.Context, productID uuid.UUID) (Bid, error) {
	row := q.db.QueryRow(ctx, getHighestBidsByProductId, productID)
	var i Bid
	err := row.Scan(
		&i.ID,
		&i.ProductID,
